...
public class Player : MonoBehaviour {
    ...
    private void FixedUpdate()
    {
            ...
            if (vel != Vector2.zero)
            {
                if (rightAnim.isActiveAndEnabled) rightAnim.SetBool("Walking", true);
                if (leftAnim.isActiveAndEnabled)  leftAnim.SetBool("Walking", true);
            }
            else
            {
                if (rightAnim.isActiveAndEnabled) rightAnim.SetBool("Walking", false);
                if (leftAnim.isActiveAndEnabled) leftAnim.SetBool("Walking", false);
            }
            ...
            {
                if (rightAnim.isActiveAndEnabled) rightAnim.SetBool("Dash", true);
                if (leftAnim.isActiveAndEnabled) leftAnim.SetBool("Dash", true);
                Dodging = true;
                playerDash.Play();
                // Debug.Log("OPA");
                TimerActive = true;
    ...
    public void Parry()
    {
        if (canParry)
        {	
        	if (rightAnim.isActiveAndEnabled) rightAnim.SetBool("Parry", true);
            if (leftAnim.isActiveAndEnabled) leftAnim.SetBool("Parry", true);
            Inimigo[] Atackers = new Inimigo[Atacker.Count];
            Atacker.CopyTo(Atackers);
            if (defenceWindow)
            {
                playerSwing.Play();
                if (rightAnim.isActiveAndEnabled) rightAnim.SetBool("Enemy Attack", true);
                if (leftAnim.isActiveAndEnabled) leftAnim.SetBool("Enemy Attack", true);
                Invoke("ReT", 0.9f);
    ...
    public void ReT()
    {
        if (rightAnim.isActiveAndEnabled) rightAnim.SetBool("Enemy Attack", false);
        if (leftAnim.isActiveAndEnabled) leftAnim.SetBool("Enemy Attack", false);
    }

    public void ReParry()
    {
        canParry = true;
        if (rightAnim.isActiveAndEnabled) rightAnim.SetBool("Parry", false);
        if (leftAnim.isActiveAndEnabled) leftAnim.SetBool("Parry", false);
    }

    public void ReDodge()
    {
        canDodge = true;
        if (rightAnim.isActiveAndEnabled) rightAnim.SetBool("Dash", false);
        if (leftAnim.isActiveAndEnabled)leftAnim.SetBool("Dash", false);
    }
